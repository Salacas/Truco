...
void enviarEstado(int opcionServer, char manoServerAux[3][17], char *nombreServer);
...
void enviarEstado(int opcionServer, char manoServerAux[3][17], char *nombreServer)
{
	//envia un string al cliente con la carta que tiro el server
	//el string es del tipo (nombreServer): "Tiro el (nombreCarta)"

	char salida[MAXDATASIZE];
	int i = 0;

	strcpy(salida, nombreServer);

	strcpy(&(salida[strlen(salida)]),": \"Tiro el ");

	strcpy(&(salida[strlen(salida)]),manoServerAux[opcionServer]);

	//coloco las comillas finales
	salida[strlen(salida)+ 1]='\0';

	salida[strlen(salida)]='"';

	//envio la string al cliente
...
int main( void )
{
	...
	char manoServerAux[3][17];//para la funcion enviarEstado
	char manoClienteAux[3][17];//para imprimir el estado en el server
	...
	while(flag != 3)//flag = 3 significa que termino el juego
	{
		...
		for(i = 0;i < 3;i++)//copio las manos a los aux
		{
		...
		if(manonumero%2 == 1)//es mano el server
		{
			...
			(primera == 2 && segunda == 2 && tercera == -1))//tira carta el server primero
			{
				...
				if(i != 1)//si no estamos en primera
				printf("%s: \"Tiro el %s\"\n\n", nombreCliente, manoClienteAux[opcionCliente]);
		...
		else//es mano el cliente
		{
			...
			(primera == 2 && segunda == 2 && tercera == -1))//tira carta el cliente primero
			{
				...
				if(i == 1)//si no estamos en la primera envia el estado 
				enviarString(" ");
				else
				enviarEstado(opcionServer, manoServerAux, nombreServer);
